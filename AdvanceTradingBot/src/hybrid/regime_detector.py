...
class MarketRegimeDetector:
    ...
    def _calculate_volatility(self, data):
        # annualized vol: std of daily returns * sqrt(trading days)
        return np.std(data['returns']) * np.sqrt(365)

    def _calculate_trend_strength(self, data):
        # assume 'trend_strength' is some series already computed
        return np.mean(data['trend_strength'])

    def _calculate_market_breadth(self, data):
        adv = data.get('advancing', 0)
        dec = data.get('declining', 0)
        return adv / (adv + dec) if (adv + dec) > 0 else 0.5
